...
def save_prediction(prediction: np.ndarray, prediction_name: str) -> bool:
    with open(f"./predictions/individual_predictions/{prediction_name}.pkl", "wb") as fid:
        logging.info(f"saving model to ./predictions/individual_predictions/{prediction_name}.pkl")
        pickle.dump(prediction, fid)


def get_predictions(model: any, x_data: pd.DataFrame) -> tuple:
...
def get_cross_val_predictions(
    ...
    cross_val_predictions = cross_val_predict(
        estimator=model, X=x_data, y=y_data, cv=5, method="predict"
    )
    try:
        logging.info(f"Trying to get cross validation probabilities for {model}")
        cross_val_probabilities = cross_val_predict(
            estimator=model, X=x_data, y=y_data, cv=5, method="predict_proba"
        )
    except:
        logging.info(f"No probabilities available for {model}")
...
def get_metrics(
    y_true, y_pred, model_name: str = "model", graph: bool = True
) -> dict[str:float]:
    ...
    for key, value in metrics.items():
        logging.info(f"{model_name} {key}: {round(value, 4)}")
...
def manage_predictions(
    ...
    if compute_predictions:
        val_predictions, val_probabilities = get_cross_val_predictions(
            model=model, x_data=x_train, y_data=y_train
        )
        test_predictions, test_probabilities = get_predictions(
            model=model, x_data=x_test
        )

        if save_predictions:
            save_prediction(
                prediction=val_predictions,
                prediction_name=f"{model_name}_val_predictions",
            )
            save_prediction(
                prediction=test_predictions,
                prediction_name=f"{model_name}_test_predictions",
            )
            if val_probabilities is not None:
                save_prediction(
                    prediction=val_probabilities,
                    prediction_name=f"{model_name}_val_probabilities",
                )
            if test_probabilities is not None:
                save_prediction(
                    prediction=test_probabilities,
                    prediction_name=f"{model_name}_test_probabilities",
                )

    else:
        val_predictions = load_file(f"./predictions/individual_predictions/{model_name}_val_predictions.pkl")
        test_predictions = load_file(f"./predictions/individual_predictions/{model_name}_test_predictions.pkl")

        if os.path.isfile(f"./predictions/individual_predictions/{model_name}_val_probabilities.pkl"):
            logging.info(f"No probability files to load for {model_name}")
            val_probabilities = load_file(
                f"./predictions/individual_predictions/{model_name}_val_probabilities.pkl"
            )
            test_probabilities = load_file(
                f"./predictions/individual_predictions/{model_name}_test_probabilities.pkl"
            )
