...
class Dataset():
    ...
    def clean_missing(self,missing_threshold:float=0.1,test_method:str='delete') -> Self:
        ...
        logging.info(f"Cleaning missing data with threshold {missing_threshold} and method {test_method}")
    ...
    def create_cuadratic_features(self)-> Self:
        if self.cleaned == False:
            raise ValueError("Data has no been split. Clean data before to reduce the number of features.")

        columns:list[str] = self.train.select_dtypes(exclude=["object"]).columns.tolist()
    ...
    def normalize(self, max_value:float=1, cols_to_ignore:list[str] = ['target'])->Self:
        if self.split == False:
    ...
    def remove_outliers(self,threshold_n_sigmas:float, cols_to_ignore:list[str]=[]) -> Self:

        if self.split == False:
            raise ValueError("Data has not been split in train/test. Cannot remove outliers.")
...
if __name__ == "__main__":
    ...
    dataset.rename_target('c1').one_hot_encode().split_train_test().clean_missing(missing_threshold=0.01).create_cuadratic_features()
